...
-- CAMBIAMOS DE NOMBRE Y/O DE TIPO PARA ALGUNAS COLUMNAS DE ALGUNAS TABLAS 
...
-- CREAMOS UNA TABLA USAMOS DECIMAL PARA DECIR CUANTO NUMERO VA A TENER (13) Y DE ESOS NUMEROS CUANTOS SON DECIMALES (10)
-- CON EL AFTER LE DECIMOS EN QUE POSICION VA LA COLUMNA 
...
-- LLENAMOS LOS ESPACIOS EN BLANCOS (VACIOS) CON (0) CUANDO SON NUMEROS Y 
-- CUANDO SON STRING LOS LLENAMOS CON GUIONES (-)
UPDATE cliente SET Y = '0' WHERE Y = '';
UPDATE cliente SET X = '0' WHERE X = '';



-- esta sentencia permite usar delete, update en (0)
SET SQL_SAFE_UPDATES = 0;


-- ACTUALIZAMOS Y REEMPLASAMOS LAS COMAS POR LOS PUNTOS  (2 POR UNO)
-- OJO ---> LOS PUNTOS SON = DECIMALES
--          LAS  COMAS SON = MILES EN SQL
UPDATE `cliente` SET Latitud = REPLACE(Y,',','.');
UPDATE `cliente` SET Longitud = REPLACE(X,',','.');

-- esta sentencia impide usar delete, update en (1)
