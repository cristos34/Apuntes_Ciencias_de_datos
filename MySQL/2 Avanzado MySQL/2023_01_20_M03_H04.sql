...
-- venta agrupado por año, trimestre y rango etario
SELECT 	ca.anio,
		ca.trimestre,
		c.Rango_Etario,
...
-- sin indices 9.5 segundos
-- con indices 0.156 segundos

-- venta agrupado por año, trimestre y rango etario 2
SELECT 	ca.anio,
		ca.trimestre,
		c.Rango_Etario,
		SUM(v.Precio * v.Cantidad) AS Venta
...
GROUP BY ca.anio, ca.trimestre, c.Rango_Etario
ORDER BY ca.anio, ca.trimestre, c.Rango_Etario;
-- sin indices .937 segundos
-- con indices .0234 segundos
...
-- sin indices 1.56 segundos
-- con indices 0.672 segundos
...
-- ALTER TABLE `empleado` ADD PRIMARY KEY(`CodigoEmpleado`); -- error de clave duplicada
...
-- DROP INDEX IdCliente ON venta; -- <-- como borramos un idice
